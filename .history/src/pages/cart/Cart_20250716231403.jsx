...
function Cart() {
  ...
  // Initialize storage
const storage = getProvider(fireDB);

  const sendPDFReceipts = async () => {
    if (!paymentDetails) return;

    const { paymentId, addressInfo, cartItems, totalAmount, shipping, grandTotal, orderDate, email } = paymentDetails;
  ...
  const storageRef = ref(storage, `receipts/${paymentId}.pdf`);
    ...
    const pdfLink = await getDownloadURL(storageRef);

    // Now send the email with this link
    emailjs.send("service_oto8pcs", "template_1tqqra7", {
      name: "Aakanshi Malik",
      to_email: paymentDetails.email,
      message: `Thanks for shopping with Celebal-Mart! Download your receipt here: ${pdfLink}`,
